...
Specs live in `.kiro/specs/<name>/` or `.windloop/specs/<name>/` — same format, auto-detected, with `.kiro` preferred when both are present. Specs are **Kiro-compatible**: the same `requirements.md` → `design.md` → `tasks.md` format and EARS acceptance criteria used by [Kiro](https://kiro.dev). Switch IDEs without losing planning artifacts.
...
**Analytic workflow?**
Use `/spec-plan` the same way, but pair this skill with `analytic-workbench`
and read `references/analytic-work.md` so review gates, experiment comparisons,
and promotion stages are part of the spec.
