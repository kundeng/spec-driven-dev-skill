...
1. Read `SPEC_DIR/requirements.md` to understand the requirements and scope. If `steering/` exists (`.windloop/steering/` or `.kiro/steering/`), read it for project-level context.
   If the spec is analytic/data-oriented, also read `references/analytic-work.md` and use `analytic-workbench` conventions where appropriate.
...
8. Implement the task:
   ...
   - For analytic tasks, preserve the exploratory vs promoted-stage boundary defined in the spec instead of ad hoc collapsing them
